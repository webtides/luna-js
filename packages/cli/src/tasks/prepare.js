...
const copyEmptyMoonConfig = async () => {
    fs.copyFileSync(path.join(global.lunaCli.currentDirectory, "src/tasks/prepare", "luna.config.example.js"), getPathToConfigFile(global.lunaCli.currentWorkingDirectory));
    ...
    fs.mkdirSync(path.join(global.lunaCli.currentWorkingDirectory, "views/layouts"), { recursive: true });
    fs.mkdirSync(path.join(global.lunaCli.currentWorkingDirectory, "assets/css"), { recursive: true });

    const filesToCopy = [
        { from: "src/tasks/prepare/page.example.js", to: path.join(settings.pages.input[0], "index.js") },
        { from: "src/tasks/prepare/layout.example.js", to: path.join(global.lunaCli.currentWorkingDirectory, "views/layouts", "base.js") },
        { from: "src/tasks/prepare/assets/main.example.css", to: path.join(global.lunaCli.currentWorkingDirectory, "assets/css", "main.css") },
    ]

    filesToCopy.forEach(({ from, to }) => {
        fs.copyFileSync(path.join(global.lunaCli.currentDirectory, from), to);
    });
};

const checkRequirements = async ({ setup } = { setup: false }) => {
    const pathToConfigFile = getPathToConfigFile(global.lunaCli.currentWorkingDirectory);
