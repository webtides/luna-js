...
const loadMoonPackageJSON = () => {
	return JSON.parse(
		fs.readFileSync(path.join(process.cwd(), 'node_modules/@webtides/luna-js', 'package.json'), {
			encoding: 'utf-8',
		}),
	);
};
...
export const rollupPluginApiExport = function ({ externals, outputDirectory }) {
	return {
		...
		async writeBundle() {
			...
			fs.writeFileSync(path.join(outputDirectory, 'package.json'), JSON.stringify(exportPackageJSON), {
				encoding: 'utf-8',
			});
		},
	};
