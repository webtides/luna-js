...
function frontmatter(markdown, opts = {
  ...
  try {
    ...
    const reason = e.reason // make this common error message a little easier to understand
    ? e.reason.startsWith('can not read a block mapping entry;') ? defaultReason : e.reason : defaultReason;
    const error = {
      reason,
  ...
  ({
    errors
  } = revalidator.validate(data, schema)); // add filepath property to each error object

  if (errors.length && filepath) {
    errors = errors.map(error => Object.assign(error, {
      filepath
    }));
  } // validate key names
...
//# sourceMappingURL=index-e78c59c6.js.map
